...
{
	// First, find an empty process descriptor.
	pid_t i;
	for (i = 1; i < NPROCS; i++)
	{
	...
	//If there is no empty process descriptor, return -1.  
	//Remember not to use proc_array[0].
	...
	//                should arrange this.
	//   * reg_eax   There is one other difference. 
	//                sys_fork() returns 0 to the child process
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// YOUR CODE HERE!
	pid_t tmp = src->p_pid;
	src_stack_top = PROC1_STACK_ADDR + tmp*PROC_STACK_SIZE;
	src_stack_bottom = src->p_registers.reg_esp;
	dest_stack_top = src_stack_top + PROC_STACK_SIZE;
	dest_stack_bottom = src_stack_top + (src_stack_top - src_stack_bottom);
	// YOUR CODE HERE: memcpy the stack and set dest->p_registers.reg_esp
	memcpy(dest_stack_bottom, src_stack_bottom, src_stack_top - src_stack_bottom);
